            ...
            context("Garmin Run") {
                ...
                context("laps") {
                    ...
                    it("should have a lap total time") {
                        expect(activity.laps?.first?.totalTime).to(beCloseTo(57.9800, within: 0.01))
                    }
                    
                    it("should have a lap total distance") {
                        expect(activity.laps?.first?.totalDistance).to(beCloseTo(165.78, within: 0.01))
                    }
                    
                    it("should have a lap maximum speed") {
                        expect(activity.laps?.first?.maximumSpeed).to(beCloseTo(2.92, within: 0.01))
                    }
                ...
                context("points") {
                    ...
                    it("should have a altitude in meters") {
                        expect(point.elevation).to(beCloseTo(14.8000, within: 0.1))
                    }
