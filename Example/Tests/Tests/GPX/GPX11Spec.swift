    ...
    override func spec() {
        fdescribe("GPX 1.0") {
            context("waypoints") {
                ...
                beforeEach {
                    let url = Bundle(for: GPX11Spec.self).url(forResource: "GPX 1.1 Waypoint", withExtension: "gpx")!
                ...
                it("should have an elevation") {
                    expect(point?.temperature) == 24
                }
                
                it("should have an heart rate") {
                    expect(point?.heartRate) == 132
                }
                
                it("should have an cadence") {
                    expect(point?.cadence) == 100
                }
            ...
            context("route") {
                var file: ImprovedFile!
                beforeEach {
                    let url = Bundle(for: GPX11Spec.self).url(forResource: "GPX 1.1 Route", withExtension: "gpx")!
                    file = try! ImprovedParser(fileURL: url).parse()
                }
            ...
            context("route with meta") {
                var file: ImprovedFile!
                beforeEach {
                    let url = Bundle(for: GPX11Spec.self).url(forResource: "GPX 1.1 Route with Meta",
                                                                  withExtension: "gpx")!
                    file = try! ImprovedParser(fileURL: url).parse()
                }
            ...
            context("track") {
                var file: ImprovedFile!
                beforeEach {
                    let url = Bundle(for: GPX11Spec.self).url(forResource: "GPX 1.1 Track", withExtension: "gpx")!
                    file = try! ImprovedParser(fileURL: url).parse()
                }
            ...
            context("track with meta") {
                var file: ImprovedFile!
                beforeEach {
                    let url = Bundle(for: GPX11Spec.self).url(forResource: "GPX 1.1 Track with Meta",
                                                                  withExtension: "gpx")!
                    file = try! ImprovedParser(fileURL: url).parse()
                }
                ...
                it("should have no average heart rate") {
                    expect(file.track?.averageHeartRate) == 131
                }
                
                it("should have no maximmum heart rate") {
                    expect(file.track?.maximumHeartRate) == 132
                }
                
                it("should have no average cadence") {
                    expect(file.track?.averageCadence) == 105
                }
                
                it("should have no average temperature") {
                    expect(file.track?.averageTemperature) == 25
                }
            }
        }
    }
